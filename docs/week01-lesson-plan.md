...
- **운영 방식**: 멘토 시연 → 코드·프롬프트 공개 → 영상 아카이브 (코드 얼롱 없음)
...
- 출석은 체크한다 — 다만 **수료 기준으로 삼지 않는다**
- 오픈소스 AI·SW 활용 인재 양성 과정 특성상 수료 여부는 멘토 재량으로 결정
- 수료 판단은 출석 대신 **오픈소스로 에이전트를 만들어 공유하는 것**으로 대체
  - 산출물 제출 흐름은 유지 (2주 연속 미제출 시 개별 면담 — 탈락이 아니라 상황 파악 목적)
