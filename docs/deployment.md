...
- 공개 주소:
  [2026-ai-service-engineering-a.github.io/ai_service_engineering-track_a](https://2026-ai-service-engineering-a.github.io/ai_service_engineering-track_a/)
- 워크플로: [.github/workflows/deploy.yml](../.github/workflows/deploy.yml)
- 트리거: `main` push, 그리고 수동 실행(workflow_dispatch)

git flow를 쓰므로 배포는 `feature/*`나 `develop`이 아니라 **release·hotfix가
`main`에 머지될 때** 일어난다. `develop`에 머지한 것만으로는 공개 사이트가
바뀌지 않는다. 브랜치 규칙은 [git-workflow.md](git-workflow.md) 참고
...
| 위치 | 값 |
| --- | --- |
| `site/astro.config.mjs`의 `site` | `https://2026-ai-service-engineering-a.github.io` |
| `site/astro.config.mjs`의 `base` | `/ai_service_engineering-track_a` |
| 실제 주소 | `https://2026-ai-service-engineering-a.github.io/ai_service_engineering-track_a/` |
...
- API 키, 개인정보, 미공개 자료를 콘텐츠나 커밋에 넣지 않는다.

## 배포가 안 될 때

| 증상 | 확인할 것 |
| --- | --- |
| `develop`에 머지했는데 사이트가 그대로 | 배포 트리거는 `main` push다. release 브랜치를 `main`에 머지했는지 |
