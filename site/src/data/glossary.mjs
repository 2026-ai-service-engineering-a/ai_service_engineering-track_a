...
export const glossary = {
  ...
  'ag-ui': {
    label: 'AG-UI',
    def: 'Agent-User Interaction Protocol. 에이전트와 사용자 화면 사이를 이벤트 스트림으로 표준화한 규격. MCP가 에이전트와 도구 사이를 맡는다면 AG-UI는 에이전트와 사람 사이를 맡는다. 실행 진행, 토큰 스트리밍, 도구 호출, 공유 상태 갱신이 전부 정해진 이벤트로 흐른다.',
  },
  'shared-state': {
    label: '공유 상태',
    def: '화면이 그리는 문서와 에이전트가 읽고 쓰는 상태가 같은 하나인 구조. 사용자가 화면에서 고친 것을 에이전트가 알고, 에이전트가 고친 것이 화면에 곧바로 나타난다. 상태 스키마에 없는 것은 에이전트가 건드릴 수 없으므로, 스키마가 곧 권한의 경계가 된다.',
  },
  'json-patch': {
    label: 'JSON Patch',
    def: 'RFC 6902. JSON 문서의 변경을 add·remove·replace·move·copy·test 연산의 배열로 표현하는 규격. 문서 전체를 다시 보내는 대신 바뀐 자리만 보낸다. AG-UI의 STATE_DELTA가 이 형식을 쓴다.',
  },
  'generative-ui': {
    label: '생성 UI',
    def: '모델의 도구 호출이 화면 컴포넌트 하나에 묶여, 호출이 일어나면 그 컴포넌트가 렌더되는 방식. 모델이 마크업을 만드는 것이 아니라 우리가 미리 만들어 둔 컴포넌트 중에서 고르고 인자만 채운다. 그래서 디자인과 타입이 깨지지 않는다.',
  },
  'frontend-tool': {
    label: '프런트엔드 도구',
    def: '서버가 아니라 브라우저가 실행 주체인 도구. 에이전트가 호출하면 화면에서 컴포넌트가 뜨거나 사용자에게 승인을 묻고, 그 결과가 도구 결과로 되돌아가 루프가 이어진다. 생성 UI와 승인 카드가 같은 메커니즘의 두 용도다.',
  },
};
