...
description: "6주차 라이브 세션 상세 교안: examples/04_react 4종으로 ReAct 루프와 트레이스, examples/05_loops 6종으로 Plan-Execute·재계획·Reflexion·ReWOO, examples/06_harness 10종으로 인젝션 방어와 budget guard, examples/07_mcp 4종으로 MCP, 그리고 v0.3에서 v1.2까지"
summary: "라이브 세션의 상세 교안입니다. 5주차에서 도구를 든 에이전트에 루프를 달아 처음으로 동작시킵니다. 트레이스를 읽고, 스텝 한도가 무한 루프를 막는 것을 재현하고, 컨텍스트가 스텝마다 부푸는 것을 실측합니다. 그 비용 구조를 공격하는 루프들을 견준 뒤, 인젝션 사고를 재현하고 데이터 경계·권한 게이트·budget guard로 막아 에이전트를 v1.0으로 완성합니다. 마지막으로 도구를 표준 프로토콜로 주고받는 MCP를 맛봅니다."
date: 2026-08-28
category: foundation
slides: ["session-06"]
tags: ["ReAct", "트레이스", "스텝 한도", "Plan-and-Execute", "Reflexion", "ReWOO", "하네스", "프롬프트 인젝션", "budget guard", "MCP", "v1.0"]
level: 2
order: "06"
---
...
> 라이브 세션의 상세 교안입니다. 5주차와 같은 실습 랩이고, 5주차가 끝난
> 지점(v0.2)에서 이어집니다. 예제 24종을 하나씩 실행합니다.
...
이번 주 저장소는 5주차 랩의 연장입니다. `agent/`와 `tests/`가 5주차의 v0.2와
같고, `examples/`에는 이번 주 몫 24종이 더 들어 있습니다.
...
docker compose exec lab pytest        # 시작 지점의 테스트 16개 통과
```

### 2-2. 이번 주 사다리는 네 단

**첫 태그가 v0.1이 아니라 v0.2입니다.** 5주차가 끝난 지점이 여기의 출발점이라
번호를 이어 받았습니다.

| 릴리즈 | 상태 | 테스트 | 대응 |
| --- | --- | --- | --- |
| v0.2 | 5주차 종료 지점: 도구는 있고 루프는 없다 | 16개 | 시작점 |
| v0.3 | [[react\|ReAct]] 루프 완성: 여행 플래너가 처음 동작 | 17개 | 3장 |
| v0.4 | 고급 루프: [[reflexion\|Reflexion]] 재시도 장착 | 21개 | 4장 |
| v1.0 | [[harness\|하네스]] 완성: 인젝션 방어 + budget guard | 33개 | 5장 |
| v1.1\~v1.2 | [[mcp\|MCP]] 시연 4종 (stdio·HTTP 전송) | 33개 | 6장 |
...
교재는 `examples/04_react/` 4종이고, 이 장 끝에서 **v0.3**으로 오릅니다.
이 4종은 에이전트 본체를 사용하므로 v0.3 이상에서 동작합니다. 이전 태그에서
실행하면 checkout 안내가 나옵니다. 아래에는 4종 전부의 실제 실행 결과가
실려 있습니다.
...
교재는 `examples/05_loops/` 6종이고, 이 장 끝에서 **v0.4**로 오릅니다.
...
교재는 `examples/06_harness/` 10종이고, 이 장 끝에서 **v1.0**, 완성된
에이전트입니다. **10종을 전부 실행합니다.** 아래에는 10종 전부의 실제 실행
...
### 5-12. v1.0 코드 포인트: 하네스가 루프에 물리는 지점
...
실행합니다. 아래에는 4종의 실제 실행 결과가 실려 있습니다. v1.1에 3종이,
v1.2에 HTTP 전송 시연이 추가되었습니다.
