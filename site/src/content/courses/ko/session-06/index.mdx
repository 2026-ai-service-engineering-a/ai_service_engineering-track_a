...
이번 주 저장소는 5주차 랩의 연장입니다. 5주차의 예제 32종이 그대로 있고,
...
- **재시도도 같은 장부를 씁니다**: 기본 실행은 Reflexion 재시도를 포함하고,
  재시도는 곧 재호출입니다. `main.py`가 guard·logger를 재시도 경로에도
  넘기는 이유입니다. 하네스가 가장 필요한 자리가 거기입니다
...
7주차에는 오늘 만든 루프 여럿을 엮은 멀티 에이전트 구조(supervisor·handoff·
병렬)를 다루고, 8주차에 FastAPI로 서비스화합니다. 오늘 만든 루프와 하네스가
그때 서버 안으로 들어갑니다.
