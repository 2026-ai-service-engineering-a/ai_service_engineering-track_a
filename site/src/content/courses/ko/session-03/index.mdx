...
| #4 | `feature/progress-and-audit` | `POST /plan/stream`([[sse\|SSE]])로 파이프라인 진행 이벤트를 흘리고, 응답에 코드 재계산 근거 `audit`을 싣는다. UI에 진행 로그·검증 상세 |
...
**진행 이벤트 ([[sse|SSE]])**: UI가 쓰는 엔드포인트를 curl로도 볼 수 있습니다.
