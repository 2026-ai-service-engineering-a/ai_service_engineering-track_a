...
실행합니다. 회전을 따라왔다면 자기 결과물로 돌리면 되고, 문서와 같은
출력·로그 마커를 원하면 3-1과 같은 방법으로 태그를 v1.0으로 바꿔 내려받아
그 폴더에서 띄웁니다. `.env`는 작업 폴더의 것을 복사합니다.

프롬프트로 만든 내 결과물은 파일 이름과 로그 마커(`PLANNER PROMPT` 등)가
문서와 다를 수 있습니다. 요구사항을 만족하면 그것대로 성공이고, 내 코드에서
아래 장면들의 대응물을 찾아보는 것도 좋은 연습입니다.
...
발행되어 있습니다. 3-1과 같은 방법으로 태그를 v1.5로 바꿔 내려받아
그 폴더에서 띄웁니다.
