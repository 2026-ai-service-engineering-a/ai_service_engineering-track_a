...
- **다운로드 대상**: "음식 DB", 약 11.35MB에 2만 건 규모입니다. 엑셀 파일 다운로드 방식이라 Open-API 제한과 무관합니다.
...
**직접 돌려 보기.** 내려받은 엑셀을 `data/food_data.xlsx`로 이름을 바꿔 넣고
전처리를 한 번 실행하면, 위 그림이 그대로 돌아 `data/foods.json`이 만들어집니다.

```bash
# K-FIND에서 받은 "음식 DB" 엑셀을 data/food_data.xlsx로 이름 변경한 뒤
...
- 2만 행을 다 담지 않고 **500종만 샘플링**합니다. 이 목록은 계획자 프롬프트에 통째로 들어가므로 컨텍스트에 실리는 크기여야 합니다. 데이터가 이보다 커지면 목록 주입 대신 검색이 필요해지고, 그것이 4\~6주차의 주제입니다.
...
- v0.1에는 이미 만들어진 `foods.json`이 들어 있습니다. 이 단계를 건너뛰고 바로 시연을 재현해도 되고, 위 명령은 **원본에서 다시 만들어 보는 경로**입니다.
...
- 역할이 곧 설정입니다.
  - 코드 어디에도 모델명이 없습니다. 조합 교체는 env 수정 + 재실행입니다 (시연 4에서 실증)
...
**시작점 내려받기.** 저장소를 클론하지 않고 **v0.1 태그의 ZIP을 내려받아**
시작합니다. 클론하면 완성본 이력까지 딸려 와 정답을 먼저 보게 되고, 무엇보다
**첫 커밋을 내가 찍는 것**이 이번 주 Git Flow 시연의 출발점이기 때문입니다.

저장소 페이지에서 ① 태그 선택기를 **v0.1**로 바꾸고 ② `Code` → ③ `Download ZIP`:

![](download_code_0.1.png)

내려받은 ZIP을 풀고, 그 폴더를 내 git 저장소로 만듭니다.

```bash
cd week03-meal-planner-0.1      # 압축을 푼 폴더

git init
git add -A
git commit -m "chore: v0.1 시작점"

git flow init -d                # main·develop과 feature/ 접두사를 기본값으로 설정
```

- `git flow`는 git에 기본 포함되지 않은 별도 도구입니다. macOS는 `brew install git-flow-avh`로 설치합니다. 설치가 여의치 않으면 `git switch -c develop`으로 develop 브랜치를 직접 만들고 `feature/*`도 손으로 만들면 됩니다 (git flow는 그 관례를 자동화해 주는 도구일 뿐입니다)
- `-d`는 브랜치 이름과 접두사를 기본값으로 두고 질문을 건너뛰는 옵션입니다. 4·5장에서 만들 `feature/planner`·`feature/validator-loop`가 이 설정을 따릅니다.
- 이 세 줄이 중요한 이유는 4장부터의 회전이 **내 저장소의 내 커밋**으로 쌓이기 때문입니다. 남의 이력을 이어받는 것이 아니라, v0.1이라는 백지에서 내가 만들어 가는 기록이 됩니다.
- 4·5장의 PR·CI까지 재현하려면 내 GitHub 계정에 빈 저장소를 하나 만들고 `git remote add origin <내 저장소 URL>`로 연결한 뒤 push합니다. 로컬 커밋만 쌓아도 회전 자체는 그대로 따라갈 수 있습니다.

**완성본과 비교하기.** 막히거나 결과를 대조하고 싶으면 같은 방법으로 태그를
**v1.0**으로 바꿔 한 번 더 내려받아, 별도 폴더에 풀어 두고 견줍니다.

![](download_code_1.0.png)

```bash
diff -r week03-meal-planner-0.1 week03-meal-planner-1.0 | less
```

- v1.0은 정답이 아니라 **참고 구현**입니다. 프롬프트로 만드는 방식이라 파일 분해나 이름이 달라지는 것이 정상이고, 요구사항을 만족하면 성공입니다 (1주차와 같은 원칙)

1. 위처럼 준비했으면 `.env.example`을 복사해 `.env`를 만들고 프로바이더 키를 채웁니다. 아래 명령으로 api·ui 두 서비스가 뜹니다 (2주차 관례 그대로, DB만 없음)
...
- 여기서 두 가지를 검증합니다. 식품코드를 foods.json에서 `grep`으로 확인하고(실존 여부), 환산값 하나를 암산으로 대조합니다. **틀린 끼니가 나오면 2회전의 복선입니다.** few-shot을 줬는데도 틀립니다. 프롬프트만으로 산수를 보증할 수 없습니다.
...
조직 소유. 시작점 v0.1과 완성본 v1.0을 태그로 발행해, 재현자가 필요한 지점의
ZIP을 내려받게 합니다 (3-1 참고)
...
│   └── Dockerfile
├── data/
│   ├── foods.json           # 식약처 영양DB 스냅샷 500종 (전처리 완료, 출처 명기)
│   └── food_data.xlsx       # 내려받은 원본 자리 (.gitignore — 커밋하지 않음)
├── scripts/
│   └── prepare_data.py      # 원본 엑셀 → foods.json 전처리 (prepare-data 서비스가 실행)
├── examples/
│   ├── naive_plan.py        # 0단 나쁜 프롬프트 (의도된 실패 예제)
│   └── naive_parse.py       # 0단 출력을 json.loads 시도 → 실패 확인용 (3줄)
├── docker-compose.yml       # api + ui, 그리고 일회성 prepare-data (DB 없음)
...
- foods.json은 시연 규모에 맞게 500종으로 샘플링한 스냅샷입니다. 프롬프트에 후보 목록을 통째로 넣을 수 있는 크기이자, 레포에 부담이 없는 크기입니다 (2-3장 전처리 절차 참고)
...
- **환산 실수가 재현되지 않을 수 있습니다.** 1회전의 "few-shot을 줘도 틀린다"는 계획자 모델의 성능에 따라 안 나올 수 있습니다. 안 나오면 그것대로 정상이고, 대신 5장 크로스체크에서 코드와 LLM의 역할 분담을 확인하면 됩니다.
...
- 재현 과제: week03 재현입니다. v0.1 ZIP을 내려받아 `git init` · `git flow init`으로 내 저장소를 만든 뒤(3-1 참고), 2주차에 설치한 [[docker|Docker]]로 `compose up`부터 시작합니다.
