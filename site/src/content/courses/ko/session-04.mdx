...
- 전달 멘트: "2주차엔 왜 PostgreSQL이었고 3주차엔 왜 파일이었고 오늘은 왜 SQLite인가. 이 세 답을 갖고 있으면 10주차 기획에서 저장 고민이 끝납니다"
...
- 멘트: "v0.1이 v1.0이 되는 순간은 코드가 아니라 **이 화면이 채워지는 순간**입니다"
...
- 마무리: "오늘 파이프라인은 전부 이 샘플 1,000건으로 개발하고, 전체 31만 건은 과제에서 돌립니다. **큰 데이터는 작은 데이터로 개발합니다**"
...
- 검산을 소리 내어 합니다: "1,000 = 987 + 13. 데이터가 새지 않았습니다"
...
- 멘트: "빠진 13건이 어디서 왜 빠졌는지 파일로 남습니다. 나중에 '데이터 왜 줄었어요?'에 대한 답이 여기 있습니다"
...
- 비용 산수를 그 자리에서 합니다: "건당 약 $0.0007이니 31만 건 전량이면 약 $210입니다. 규칙이 85%를 공짜로 막아주니 잔여만 돌리면 약 $30. **정규식 한 뭉치의 가격이 $180**입니다"
...
- 멘트: "규칙은 이 문장 앞에서 손을 들었고, LLM은 다섯 형태를 전부 갈라냈습니다. 각자의 자리입니다"
...
- 멘트: "두 번째 실행은 공짜입니다. 배치는 죽고 다시 살아나는 것까지가 설계입니다. 31만 건 돌리다 끊겨도 이어 달립니다"
...
- 3주차 연결 멘트: "지난주 instructor는 서비스 안에 있었습니다. 오늘은 파이프라인 안에 있습니다. structured output은 어디서든 'LLM을 함수처럼' 쓰는 도구입니다"
...
- 멘트: "환산값 1,852mg. 2회전에서 LLM이 갈라낸 '생·숙면 200g'이 여기서 일합니다. 그리고 모르는 건 모른다고 답합니다. 정직한 NULL이 사용자 신뢰입니다"

**UI 전환**

- UI를 새로고침하면 **빈 상태 화면이 검색 화면으로** 바뀝니다. UI 코드는 v0.1 그대로입니다 (헤드리스 분리의 증명, 3주차와 동일한 장면)
- 검색 → 나트륨 정렬 → 상세에서 1회 제공량 환산을 확인합니다
- 멘트: "1회전의 정제, 2회전의 파싱이 없었으면 이 화면의 숫자는 전부 거짓말이거나 빈칸이었습니다"
...
- **원본 다운로드를 가장 먼저**: 187MB 파일을 교안 확정 즉시 확보하고, 버전(2026-07-28판)·다운로드 일자를 기록합니다. 샘플 1,000건도 이때 추출하되, 더러움 유형이 골고루 들어가게 의도적으로 선별합니다 (BOM·깨진 문자·복합 섭취참고량·빈 값 포함 행 보장)
- **엑셀 로딩 시간**: 187MB xlsx는 pandas 로드가 느릴 수 있습니다. 리허설에서 시간을 측정하고, "전체 실행은 과제" 원칙을 유지합니다 (시연은 샘플로만)
- **커버리지·비용 수치는 자리표시자**: 규칙/LLM/실패 비율(85/3/12), 건당 비용, 전량 대비 절감액은 리허설 실측으로 교체합니다. 로그 마커(`PARSER PROMPT` 등)와 grep 명령도 실제 구현의 마커에 맞춰 조정합니다
- **출력 예시 전반이 자리표시자**: 본 문서의 명령 출력 블록은 리허설 실물 캡처로 교체하는 것을 전제로 합니다
- 사전 리허설·체크포인트 브랜치·10분 손절, compose 이미지 사전 pull, 키·쿼터 확인은 관례대로 합니다
- **UI 사전 구축 완성도**: 빈 상태와 채워진 상태 양쪽을 리허설에서 확인합니다 (3주차 리스크와 동일)
...
- 연결 멘트: "오늘 검색은 키워드였습니다. '떡볶이'라고 쳐야 떡볶이가 나옵니다. '매콤한 분식 간식'으로 찾고 싶다면? 그게 임베딩이고, 다음 주입니다"
