...
flowchart LR
    ...
    M --> C
    C -->|"모델 호출"| G["litellm :4000<br/>게이트웨이"]
    G --> L(("모델 제공자"))
```
...
그림의 오른쪽 끝을 보면 우리 코드와 모델 제공자 사이에 `litellm :4000`이
...
```mermaid
flowchart LR
    B1["api"] -->|"가상키 A"| G["게이트웨이<br/>litellm :4000"]
...
> 것은 **라이브러리**이고, 여기 `:4000`에 뜬 것은 같은 프로젝트의
> **프록시 서버**입니다. 앱은 여전히 라이브러리로 호출하고, 그 호출이 향하는
> 주소만 프록시로 바뀝니다.
...
docker compose up --build -d  # api · ui · mcp · n8n · litellm · db
