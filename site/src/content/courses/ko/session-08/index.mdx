...
### 3-4. 코드 지도: 무엇이 무엇을 부르는가

앞의 세 절이 말로 한 것을 코드에서 확인합니다. 아래 지도는 손으로 그린 것이
아니라 **저장소를 파싱해 만든 것**입니다. 심볼과 호출 관계가 실제 코드에서
나왔으므로, 코드가 바뀌면 지도도 바뀝니다.

<CodeMap />

몇 개만 눌러 보면 이번 주의 문장이 그대로 보입니다.

- `plan`을 누르면 **부르는 곳이 셋**입니다. HTTP 라우트 둘과 MCP 도구 하나.
  문이 늘어도 코어는 그대로라는 말이 이 화살표 셋입니다.
- `plan_events`를 누르면 `plan`과 `plan_stream`이 함께 나옵니다. 스트리밍과
  동기 응답이 같은 코드를 돈다는 3-2절의 증거입니다.
- **코어의 어느 심볼을 눌러도 부르는 것 목록에 `api`나 `mcp_server`가
  없습니다.** import 방향이 한쪽이라는 3-1절의 증거입니다.
- `completion`을 누르면 코어 안에서 모델로 나가는 길이 한 곳뿐이라는 것이
  보입니다. 게이트웨이를 끼워 넣을 자리가 거기 하나였던 이유입니다.
