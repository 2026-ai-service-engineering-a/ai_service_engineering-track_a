...
이 지도에는 **AI** 표시가 하나도 없습니다. 표시는 *그 함수 안에서* 모델을
부를 때만 붙기 때문입니다. `plan`도 `plan_events`도 `run_iter`도 남에게
시킬 뿐, 자기 손으로 모델을 부르지는 않습니다.

부르는 자리는 저장소 전체에서 **한 곳**입니다.
...
표시는 손으로 붙인 것이 아니라 `litellm`의 completion 호출을 파서가 찾아낸
결과입니다. 그래서 나중에 누가 다른 곳에서 모델을 부르면 지도에 표시가 하나
더 생깁니다. **표시가 둘이 되는 순간이 경계가 새는 순간입니다.**
