...
말로만 하면 믿기 어려우니 직접 확인합니다. 앱에서 프로바이더 키를 **전부
지우고** 가상키만 남겨 봅니다.

```bash
docker compose exec \
  -e GEMINI_API_KEY= -e OPENAI_API_KEY= -e ANTHROPIC_API_KEY= \
  -e GATEWAY_URL=http://litellm-proxy:4000 -e GATEWAY_KEY=sk-… \
...
진짜 키가 하나도 없는 컨테이너가 진짜 모델의 답을 받았습니다. 키는 게이트웨이
컨테이너에만 있습니다.
