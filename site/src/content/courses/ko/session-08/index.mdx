...
```json
{"mode":"offline","model":"offline/scripted-planner","available_providers":[],
 "defaults":{"max_steps":6,"max_cost_usd":0.2}}
```

키를 채우면 `mode`가 `live`로 바뀝니다. 코드는 한 줄도 바뀌지 않습니다.
...
오른쪽 위의 **AI** 표시는 그 심볼을 부르면 결국 모델 호출까지 이어진다는
뜻입니다. 코어에서 모델로 나가는 문이 `core/llm.py`의 `completion` 하나뿐이라
기계적으로 계산됩니다. 표시가 없는 심볼은 아무리 불러도 돈이 나가지 않습니다.
...
docker compose restart n8n     # 발행 상태는 재시작 후에 반영된다
```
...
### 7-3. 거울: 손으로 짠 루프가 항목 하나가 되어 있다

n8n의 AI Agent 노드를 열면 이런 것들이 들어 있습니다.

```plaintext
ConversationalAgent  OpenAiFunctionsAgent  PlanAndExecuteAgent
ReActAgent           SqlAgent              ToolsAgent
```

6주차에 손으로 짠 [[react\|ReAct]]와 [[plan-execute\|Plan-and-Execute]]가
남의 도구에서는 **항목 하나**입니다. 5주차의 LiteLLM 도입, 7주차의 LangGraph
도입과 같은 자리입니다. 바닥을 본 다음에 추상화를 보는 것.

한 가지 더 흥미로운 것이 있습니다. 최신 n8n에서 이 선택지는 **Tools
Agent 하나로 수렴**했고, 나머지는 옛 버전 노드에 남아 있습니다. 6주차
4장에서 루프의 스펙트럼을 보며 "적응력과 비용 사이"라고 했던 그 스펙트럼이,
현장에서는 도구 호출 기반 하나로 좁혀지는 중이라는 뜻입니다. 이런 판단을
읽으려면 스펙트럼을 알아야 합니다.
...
**기본은 꺼져 있습니다.** `.env`의 `GATEWAY_*` 셋이 비어 있으면 앱은
프로바이더를 직접 부릅니다. 랩에 게이트웨이가 떠 있어도 그렇습니다. 셋을
채우면 **같은 코드가** 게이트웨이를 지나갑니다.

```bash
# .env
GATEWAY_URL=http://litellm-proxy:4000   # compose 서비스 이름. localhost가 아니다
GATEWAY_KEY=sk-…                        # 가상키. 발급은 다음 절에서
GATEWAY_MODEL=travel                    # gateway/config.yaml의 model_name
```
...
2. `:4000/ui/`에서 **+ Create New Key**. 별명·상한·허용 모델을 정합니다.
   키 문자열은 **만든 직후 한 번만** 보여주므로 그때 복사합니다.
3. `.env`의 `GATEWAY_KEY`에 붙이고 `GATEWAY_URL`·`GATEWAY_MODEL`도 채웁니다.
...
마스터 키가 이 화면의 비밀번호라는 것도 봐 두세요. **마스터 키를 들고 있으면
키를 찍어낼 수 있고 남의 사용액을 볼 수 있습니다.** 앱에 주지 않는 이유가
이것입니다.

> 실제 배포에서는 `admin`과 마스터 키를 그대로 쓰지 않습니다. `UI_USERNAME`과
> `UI_PASSWORD`를 따로 두거나 SSO를 붙입니다. 이 화면이 공개 주소에 열려
> 있으면 게이트웨이를 통째로 내준 것과 같습니다.
