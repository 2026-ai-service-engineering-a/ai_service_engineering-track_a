...
flowchart LR
    ...
    EM --> VEC[("data/chroma/<br/>Chroma 인덱스")]
...
- 벡터 인덱스는 [[vector-db|벡터 DB]]인 Chroma의 **임베디드 모드**로 `data/chroma/` 디렉터리에 삽니다. 서버가 아니라 api 컨테이너 안의 라이브러리라, 의미 검색을 켜도 compose 서비스는 늘지 않습니다.
...
- 4회전의 벡터 인덱스도 같은 결정 구조를 따릅니다. 수천 건이면 SQLite 확장(sqlite-vec)이나 메모리 전수 비교로도 되지만, 31만 건 의미 검색에는 ANN(근사 최근접) 인덱스가 필요합니다. Chroma를 **임베디드 모드**(서버 없이 라이브러리로 로컬 디렉터리에 저장)로 쓰므로, "의미 검색을 켠다"가 "서비스를 하나 더 운영한다"가 되지는 않습니다.
...
- 임베딩 인덱스 구축: foods_parsed.parquet의 식품명·분류(대/중)·제조사를
  ...
  벡터화하고, Chroma 임베디드 컬렉션(data/chroma, 서버 없음)에 저장
...
[done ] Chroma 컬렉션 foods 987건 → data/chroma/
```
...
│   └── chroma/              # 벡터 인덱스 자리 (.gitignore — 4회전이 채움)
├── scripts/
│   ├── download_file.py     # K-FIND 파일 다운로드 (기본 권장 경로)
│   └── download_api.py      # Open-API 경로 (인증키 필요, 09~19시 제한 안내 출력)
├── docker-compose.yml       # api + ui (SQLite·Chroma 모두 api 안의 파일·라이브러리라 서비스 아님)
...
- **벡터 저장의 위아래**: 이번 주는 Chroma 임베디드였습니다. 수천 건 규모면 sqlite-vec(SQLite 확장이라 벡터가 food.db 안에 들어갑니다) 같은 더 가벼운 구성도 되고, 수백만 벡터·동시 쓰기·분산이 필요해지면 Chroma 서버 모드나 Qdrant·pgvector로 올라갑니다. 저장 3단 선택 기준의 벡터판입니다.
