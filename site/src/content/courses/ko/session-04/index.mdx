...
- **주제**: 데이터 수집·정제와 RAG 검색 (더러운 실데이터를 서비스 가능한 데이터셋으로, 데이터셋을 의미 검색으로)
- **세션 포커스**: 더러움 3종 관찰, Git Flow 4회전(정제·파싱·SQLite 서비스·RAG 검색), 규칙 우선·LLM 잔여, 검색 우선·LLM은 답변만
...
- 벡터 인덱스도 같은 food.db 파일 안에 삽니다. sqlite-vec 가상 테이블이라, 의미 검색을 켠다고 서비스가 늘지 않습니다.
...
- 4회전의 벡터 인덱스도 이 선택을 바꾸지 않습니다. sqlite-vec는 같은 food.db 파일 안의 가상 테이블이라, "의미 검색을 켠다"가 "서비스를 하나 더 운영한다"가 되지 않습니다.
...
- PR → CI → 머지. UI가 살아났지만 아직 v1.0이 아닙니다. 마지막 회전이 남았습니다.

## 7. 시연 5: Git Flow 4회전 (`feature/rag-search`)

3회전의 마지막 curl이 남긴 0건에서 시작합니다. 정제 데이터셋에 구글 임베딩
벡터 인덱스를 세워 의미 검색을 열고, 검색이 추린 몇 건만 컨텍스트로 LLM을
불러 질문에 답합니다. **검색 우선, LLM은 답변만.**
...
- 임베딩 인덱스 구축: foods_parsed.parquet의 식품명·분류(대/중)·제조사를
  ...
  벡터화하고, sqlite-vec 가상 테이블로 food.db 안에 저장
...
- 작업 단위마다 커밋 — 임베딩 인덱스 → 의미 검색 → /ask 순
```

줄별 의도 포인트:

- "식품명·분류·제조사를 한 문장으로": 무엇을 임베딩하느냐가 검색 품질을 정합니다. 영양 수치는 벡터에 넣지 않습니다. 숫자 필터는 SQL의 일이고, 여기서도 각자의 자리가 있습니다.
...
| 파일 | 보는 곳 | 왜 중요한가 |
| --- | --- | --- |
| `pipeline/embed_index.py` | 배치 루프 + 벡터 테이블 적재 | "2회전 배치 러너와 같은 예의입니다. 재개·상한·진행 로그" |
| `api/routes/semantic.py` | 질의 임베딩 → top-k 근접 검색 | "검색은 벡터 연산 하나. LLM이 없습니다" |
...
[done ] vec_foods 987행 → data/food.db
```
...
- 커밋 이력(임베딩 인덱스 → 의미 검색 → /ask 순) 확인 → PR → CI → 머지 → v1.0 태그 + PROMPT.md(지시 4개) 확인
...
│   ├── Dockerfile
│   └── pyproject.toml       # pandas, pyarrow, litellm, instructor, fastapi, pytest, openpyxl, sqlite-vec
├── ui/
...
- **전용 [[vector-db|벡터 DB]]**: sqlite-vec로 부족해지는 순간(수백만 벡터, 복잡한 필터 결합, 분산)이 오면 Chroma·Qdrant·pgvector 같은 전용 벡터 DB로 넘어갑니다. 저장 3단 선택 기준의 벡터판입니다.
