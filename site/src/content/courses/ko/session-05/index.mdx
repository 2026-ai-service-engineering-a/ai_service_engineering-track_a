...
description: "5주차 라이브 세션 상세 교안: AI 모델과 AI 서비스의 구분, 도구의 정의, examples/01_api 11종으로 LLM API의 바닥, examples/02_litellm 10종으로 통합 레이어, examples/03_tools 11종으로 tool calling 심화, 그리고 에이전트가 처음으로 도구를 드는 지점까지"
summary: "라이브 세션의 상세 교안입니다. 1~4주차에 프레임워크와 라이브러리 뒤에 가려 있던 것을 이번 주에 바닥부터 헤칩니다. 먼저 AI 모델(뇌)과 AI 서비스(도구·루프를 둘러 붙인 제품)를 가르고, 같은 질문을 도구 없이·도구를 쥐여주고 두 번 물어 그 차이를 실측으로 봅니다. 이어서 최소 호출의 요청·응답 JSON 원문에서 출발해 역할·토큰·무상태·비용을 잡고, 통합 레이어 위에서 교체·폴백·캐싱·라우팅을 쓰고, 도구 호출 왕복을 JSON 원문으로 해부합니다. 예제 32종을 전부 실행하며, 세션 끝에서 여행 플래너 에이전트가 처음으로 도구를 듭니다."
date: 2026-08-21
category: foundation
slides: ["session-05"]
tags: ["AI 서비스", "에이전트", "도구", "LLM API", "토큰", "무상태", "LiteLLM", "폴백", "tool calling", "Pydantic", "구조화 출력"]
level: 2
order: "05"
---

import ModelVsService from '../../../../components/ModelVsService.astro';

> 라이브 세션의 상세 교안입니다. 이번 주는 시연 저장소가 서비스가 아니라
> **실습 랩**입니다. 예제 32종을 하나씩 실행하며 원리를 손으로 확인합니다.
...
- **모델과 서비스를 가른다**: AI 모델은 텍스트를 넣으면 텍스트가 나오는
  함수 하나이고, 우리가 쓰는 제품은 그 위에 도구·루프·방어선을 둘러 붙인
  것입니다. 그 차이를 같은 질문 다섯 개로 직접 확인합니다
...
- **에이전트가 도구를 든다**: 세션 끝에서 여행 플래너가 처음으로 도구를 듭니다
...
- 예제 32종 전부를 수업에서 실행합니다. 이 문서에 실행 결과가 함께 실려
  있어, 재현할 때 자기 출력과 대조할 수 있습니다
- 유닛 테스트는 키·네트워크 없이 돕니다: `docker compose exec lab pytest`
- 저장소는 **완성된 한 벌**입니다. 중간 버전을 오가지 않고 `main` 그대로
  실행합니다. 이 랩의 에이전트는 도구까지 실린 상태이고, 루프는 6주차 몫입니다
...
- 예제 32종을 전부 따라 돌리고 에이전트를 여러 번 실행해도 하루 $1
  안쪽입니다
...
비용을 직접 계산하는 법은 4장 12절에서, 모델별 단가표는 5장 10절에서
다룹니다. 누적 비용에 상한을 거는 [[budget-guard|budget guard]]는 6주차입니다.

## 3. AI 모델과 AI 서비스

예제를 열기 전에 좌표를 하나 잡고 갑니다. 우리가 "AI"라고 뭉뚱그려 부르는
것이 사실은 두 층이고, 그 사이를 잇는 것이 이번 주의 주제인 도구입니다.

- **핵심 문장**: 모델은 뇌다. 뇌만으로는 오늘 날씨를 모른다

### 3-1. 뇌와 몸: 모델과 서비스는 다른 물건이다

한 줄로 가르면 이렇습니다.

- **AI 모델**: Claude Opus·Fable, GPT, Gemini 같은 것들. **텍스트를 넣으면
  텍스트가 나오는 함수 하나**입니다. 4장에서 그 함수의 요청·응답을 JSON
  원문으로 뜯어봅니다
- **AI 서비스**: ChatGPT, Claude Code, Codex, Cursor 같은 것들. 모델 하나를
  가운데 두고 대화 화면·도구·실행 루프·방어선을 둘러 붙인 **제품**입니다.
  [[agent|에이전트]]도 여기에 속합니다

모델은 뇌고 서비스는 몸입니다. 뇌는 아는 것을 말할 수 있지만 창밖을 보거나
손을 뻗지는 못합니다. 눈과 손에 해당하는 것이 도구이고, 그 손발을 언제 어떻게
쓸지 정하며 반복하는 것이 [[react|루프]]입니다.

| 제품 | 모델 위에 붙은 것 | 그래서 할 수 있는 일 |
| --- | --- | --- |
| 채팅 화면만 붙인 것 | 대화 UI + 히스토리 | 아는 것을 말한다 |
| ChatGPT | + 웹 검색·코드 실행·이미지 | 오늘 뉴스를 찾아 요약한다 |
| Claude Code | + 파일 읽기·쓰기·셸 + 루프 | 코드를 고치고 테스트를 돌린다 |
| 이 랩의 여행 플래너 | + 검색·이동시간·예산 도구 | 예산을 계산해 일정을 짠다 |

거꾸로도 성립합니다. 같은 제품에 더 좋은 모델을 꽂아도, 붙은 것이 없으면
여전히 오늘 날씨를 모릅니다. **모델의 성능이 아니라 붙은 것이 능력의 경계를
긋습니다.** 그림 두 장을 나란히 놓으면 그 경계가 눈에 보입니다.

#### ① 도구가 없다면: 고리가 안에서 닫힌다

```mermaid
flowchart TB
    subgraph S0["채팅만 붙인 모델"]
        direction TB
        UI0["대화 화면"] --> M0["AI 모델 · 뇌<br/>학습한 것만 안다"]
        M0 -->|"아는 것을 말한다"| UI0
    end
    M0 -. "닿을 길이 없다" .-> W0["바깥 세상<br/>웹·DB·파일·시계"]
```

질문이 들어오고 답이 나가는 고리가 **상자 안에서 닫힙니다**. 학습이 끝난
시점 이후의 일, 내 컴퓨터의 파일, 지금 이 순간의 시각은 저 상자 안에
없습니다. 없는 것을 물으면 모델은 아는 척을 하거나 모른다고 물러섭니다.

#### ② 도구가 있으면: 고리가 바깥까지 뻗는다

```mermaid
flowchart TB
    subgraph S["AI 서비스 (제품)"]
        direction TB
        UI["대화 화면"] --> M["AI 모델 · 뇌<br/>텍스트 → 텍스트"]
        M -->|"① 실행 요청"| H["하네스<br/>권한·검증·한도"]
        H -->|"② 통과한 것만"| T["도구<br/>검색·파일·계산·예약"]
        T -->|"④ 관찰 결과"| M
    end
    T -->|"③ 바깥과 닿는<br/>유일한 통로"| W["바깥 세상<br/>웹·DB·파일·시계"]
    W --> T
```

같은 뇌인데 고리가 바깥까지 뻗었습니다. 바뀐 것은 모델이 아니라 **모델이
부를 수 있는 것이 생겼다**는 점뿐입니다. 그리고 그 통로는 하나입니다.
모델 자리에 무엇을 꽂든 바깥과 닿는 길은 도구를 지납니다.

### 3-2. 직접 확인: 채팅만 붙인 모델이 못 하는 일

설명보다 물어보는 편이 빠릅니다. 아래는 질문 다섯 개를 같은 모델에 두 번씩
물은 실제 기록입니다. 한 번은 도구 없이, 한 번은 도구 네 개를 쥐여주고.

<ModelVsService />

#### 결과 읽기

- **모델은 대체로 모르는 것을 압니다**: 날씨·시각·파일 목록 질문에서는
  "실시간 정보에 접근할 수 없다"고 스스로 밝히고 물러섰습니다. 이런 거절은
  안전한 실패입니다
- **위험한 쪽은 거절하지 않는 질문입니다**: `3847 × 29183`에 모델은 망설임
  없이 **112,267,021**을 내놨습니다. 정답은 112,267,001입니다. 스무 개
  차이라 눈으로는 그럴듯하고, 어디에도 "확실하지 않다"는 신호가 없습니다.
  도구를 쥐여주자 계산은 코드가 했고 값이 맞았습니다
- **도구를 붙였다고 매번 부르지는 않습니다**: 여행 추천 질문에서는 도구
  네 개를 쥐여줬는데도 `tool_calls`가 비어 있었습니다. 학습된 지식으로 충분한
  질문이라 모델이 부르지 않기로 판단한 것입니다. 도구는 능력의 확장이지
  의무가 아닙니다

세 줄로 줄이면 이렇습니다. **모델은 자기 안에 없는 것을 못 하고, 못 한다는
것을 늘 알려주지는 않으며, 그 구멍을 메우는 것이 도구입니다.**

### 3-3. 도구란 무엇인가

정의부터 박아 둡니다. **도구는 모델이 이름으로 부를 수 있게 등록해 둔 우리
코드의 함수**입니다. 등록에 필요한 것은 셋뿐입니다.

- **이름**: `get_weather`
- **설명**: "도시의 현재 날씨를 조회한다"
- **입력 스키마**: `city`는 문자열이고 필수

모델에게 보이는 것은 이 셋이 전부입니다. 함수 본문은 보이지 않습니다. 모델은
"`get_weather`를 `{"city": "서울"}`로 실행해 달라"는 **요청**을 만들 뿐이고,
실행은 우리 코드가 합니다. 그래서 이름과 설명을 어떻게 쓰느냐가 곧 품질이고,
실행 앞에 무엇을 세우느냐가 곧 안전입니다. 앞의 것은 6장 11절에서 실측하고,
뒤의 것은 6주차의 게이트에서 다룹니다.
...
전부 같은 구조입니다. 모델이 요청하고, 제품의 코드가 실행하고, 결과를
되먹입니다. 6장 1절에서 이 왕복을 손으로 한 번 밟고, 2절에서 JSON 원문으로
해부합니다.

#### 도구가 아닌 것

경계를 그어 두면 나중에 헷갈리지 않습니다.

- **프롬프트에 적어 둔 지시**는 도구가 아닙니다. "계산은 정확히 해줘"는
  부탁이고, 계산기 도구는 실행입니다. 부탁은 확률로 지켜지고 실행은 코드로
  지켜집니다
- **모델 안의 지식**도 도구가 아닙니다. 오사카 명소를 아는 것은 학습의
  결과지 조회가 아닙니다
- **[[rag|RAG]]로 미리 넣어 준 문서**는 경계에 있습니다. 4주차에서 검색이
  추려 프롬프트에 넣어 준 것은 **우리가 넣어 준 것**이지 모델이 요청한 것이
  아닙니다. 같은 검색이라도 모델이 필요하다고 판단해 부르면 그때부터 도구입니다

#### 이 장이 남기는 것

- 모델은 뇌, 서비스는 도구·루프·방어선을 둘러 붙인 제품이다
- 도구는 이름·설명·입력 스키마로 등록한 우리 코드의 함수다
- 모델은 실행하지 못한다. 요청할 뿐이고 실행은 우리 몫이다

이 셋이 사실인지, 다음 세 장에서 원문과 실행 결과로 확인합니다.
...
- **참고**: 예제의 `completion()`은 [[litellm|LiteLLM]]의 통일 인터페이스입니다. 공식 SDK 대신 이것을 쓰는 이유는 5장 1절에서 다룹니다
...
기억해 두세요. 6장에서 저 자리가 채워지는 순간 에이전트가 시작됩니다.
`provider_specific_fields`처럼 프로바이더 고유 필드가 섞여 오는 것도
실물에서만 보이는 부분입니다.

### 4-2. 역할: system이 내용과 형식을 정한다
...
### 4-4. 컨텍스트 한도: 넘치면 잘리는 게 아니라 거부된다
...
### 4-9. 기억의 정체: 채팅이 나를 기억하는 이유와 compact
...
"600엔"이라는 문자열), 타입까지 묶는 방법은 6장의 구조화 출력에서 잇습니다.

### 4-11. 이미지 입력: 멀티모달도 같은 구조
...
### 5-1. 왜 LiteLLM인가: 공식 SDK 대신 통합 레이어

4장의 예제들이 이미 `from litellm import completion`을 썼습니다. 각 사가
...
- **프로바이더 고유 기능의 지연·누수**: 새 기능은 litellm이 지원할 때까지
  기다리거나, 4장 1절의 응답 원문에서 본 `provider_specific_fields`처럼
  표준 밖 필드로 새어 나옵니다. Gemini 3+의 temperature 은퇴를 경고로만
  전달하던 4장의 temperature 예제도 같은 결입니다
...
수 있다는 뜻입니다. 4장의 한도 초과 예제가 저 입력한도 열을 읽었고,
6주차의 사전 비용 추정이 저 단가 열을 읽습니다.

### 5-11. 임베딩: 같은 인터페이스의 다른 출구
...
교재는 `examples/03_tools/` 11종이고, 이 장 끝에서 **에이전트가 도구를 듭니다**.
**11종을 전부 실행합니다.** 아래에는 11종 전부의 실제 실행 결과가 실려 있습니다.
...
### 6-3. 도구의 개요: AI는 도구로 세상과 닿는다

1번과 2번이 왕복의 기계 장치를 보여줬으니, 한 발 물러서서 봅니다. 3장에서
도구가 **무엇인지** 정의했다면, 여기서는 도구가 **무엇을 메우는지**와
**만들 때 무엇을 지켜야 하는지**를 봅니다. 이 지도를 갖고 가면 뒤의 예제
여덟 개가 전부 이 지도의 한 칸씩입니다.
...
없습니다. 아래 코드 포인트에서 보듯 에이전트 본체의 도구가 전부 이 방식입니다.

### 6-5. 병렬 도구 호출: 독립 조회는 한 턴에
...
### 6-7. tool_choice: 쓸지 말지를 코드가 정한다
...
### 6-9. 인자 환각: 스키마는 통과해도 값은 허구일 수 있다
...
4장의 JSON 모드가 형태만 보장했다면, 이것은 `plan.days[0].lunch`처럼
타입까지 보장합니다. 자유 문장이 검증된 객체가 되는 순간이고, 문자열 파싱은
없습니다.

### 6-11. 도구 선택: 이름과 설명이 품질을 가른다
...
### 6-12. 도구 유무 대비: 같은 계산을 두 방식으로

코드: [`11_calc_with_without.py`](https://github.com/2026-ai-service-engineering-a/week05-agent-lab/blob/main/examples/03_tools/11_calc_with_without.py)

#### 무엇을 보는가

3장에서 그림으로 본 대비를 이 장의 문법으로 다시 봅니다. 같은 계산 요청을
① 도구 없이, ② 계산기 도구를 쥐여주고 각각 보냅니다. 자연어를 인자로 옮기는
일이 모델의 몫이고, 그 식을 실제로 세는 일이 우리 코드의 몫이라는 분업이
한 화면에 나옵니다.
...
정답은 파이썬이 미리 계산해 두고, 두 답을 각각 채점합니다.
...
```text
=== 질문: 3+4+(5 곱하기 2) 를 계산해줘 ===
  정답 (파이썬이 계산): 17

=== ① 도구 없이 — 모델이 스스로 센다 ===
  주신 수식 `3 + 4 + (5 곱하기 2)`를 계산해 드리겠습니다.
  1. 괄호 안의 연산을 먼저 계산합니다: 5 × 2 = 10
  2. 남은 덧셈을 순서대로 계산합니다: 3 + 4 + 10 = 17
  계산 결과는 **17**입니다.
  판정: ○ 맞음

=== ② 계산기를 쥐여주고 — 모델은 요청만, 실행은 우리 코드 ===
  모델의 요청  → calculator({"expression": "3+4+(5*2)"})
  우리의 실행  → {"expression": "3+4+(5*2)", "result": 17}
  최종 답: 계산 결과는 **17**입니다.
  판정: ○ 맞음

=== 질문: 48627 곱하기 9314에 1250을 더하면? 숫자로 답해줘 ===
  정답 (파이썬이 계산): 452,913,128

=== ① 도구 없이 — 모델이 스스로 센다 ===
  (48627 * 9314) + 1250을 계산한 결과는 다음과 같습니다.
  452,905,678
  판정: ✗ 틀림

=== ② 계산기를 쥐여주고 — 모델은 요청만, 실행은 우리 코드 ===
  모델의 요청  → calculator({"expression": "48627 * 9314 + 1250"})
  우리의 실행  → {"expression": "48627 * 9314 + 1250", "result": 452913128}
  최종 답: 452913128
  판정: ○ 맞음
```
...
- **번역은 모델, 계산은 코드**: `"5 곱하기 2"`라는 한국어가
  `{"expression": "3+4+(5*2)"}`로 옮겨졌습니다. 이 번역이 모델의 몫이고,
  1번에서 `"100엔이 얼마쯤이죠?"`가 `{"currency": "JPY"}`가 된 것과 같은
  일입니다. 도구가 하는 일은 그 식을 **실제로 세는 것**입니다
- **쉬운 식에서는 두 답이 같습니다**: 17로 똑같이 맞았고, 도구 없는 쪽은
  풀이 과정까지 단정하게 적었습니다. 그래서 도구 없는 쪽이 안전해 보입니다.
  **그 착시가 이 예제의 표적입니다**
- **자릿수를 키우면 갈립니다**: 정답 452,913,128에 도구 없는 쪽은
  452,905,678을 내놨습니다. 형식은 여전히 단정하고 "확실하지 않다"는 신호는
  없습니다. 다시 돌리면 **틀린 값 자체가 달라집니다**. 이 예제를 준비하며
  452,902,628도 나왔습니다. 매번 다른 오답이 나온다는 것이 계산이 아니라
  확률이라는 증거입니다

도구는 답을 바꾸는 장치가 아니라 **답의 근거를 바꾸는 장치**입니다. 쉬운
질문에서 도구가 하는 일이 없어 보인다면, 그것은 도구가 불필요하다는 뜻이
아니라 아직 틀릴 만큼 어렵지 않았다는 뜻입니다.

### 6-13. 전체 그림: 요청에서 답까지, 도구 호출의 한 바퀴
...
- **도구 목록은 매 요청 반복 전송됩니다**: 도구 스키마도 토큰입니다. 도구가
  ...
  하나 더 늘었습니다
- **결과도 그냥 메시지입니다**: 4장의 무상태 원칙이 여기서도 성립합니다.
  왕복의 기억은 전부 배열에 있습니다

### 6-14. 코드 포인트: 예산 계산 도구가 스키마로 실리는 지점
...
docker compose exec lab pytest        # 유닛 테스트 16개 통과
```

이 랩의 에이전트는 도구를 딱 1왕복만 씁니다. 여러 도구를 오가며 반복하는
루프는 6주차 랩에서 완성됩니다.
...
- 모델은 뇌다. 텍스트를 넣으면 텍스트가 나오는 함수 하나다
- 서비스는 그 위에 도구·루프·방어선을 둘러 붙인 제품이고, 에이전트도 그중 하나다
- 능력의 경계를 긋는 것은 모델의 성능이 아니라 붙어 있는 도구 목록이다
- 모델은 모르는 것을 늘 알려주지는 않는다. 산수에서 자신 있게 틀린 기록을 봤다
...
이 랩의 에이전트는 도구를 딱 한 번 씁니다. 여러 도구를 오가며 반복하려면
