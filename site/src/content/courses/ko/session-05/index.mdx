...
description: "5주차 라이브 세션 상세 교안: AI 모델과 AI 서비스의 구분, 도구의 정의, examples/01_api 11종으로 LLM API의 바닥, examples/02_litellm 10종으로 통합 레이어, examples/03_tools 10종으로 tool calling 심화, 그리고 에이전트가 처음으로 도구를 드는 지점까지"
summary: "라이브 세션의 상세 교안입니다. 1~4주차에 프레임워크와 라이브러리 뒤에 가려 있던 것을 이번 주에 바닥부터 헤칩니다. 먼저 AI 모델(뇌)과 AI 서비스(도구·루프를 둘러 붙인 제품)를 가르고, 같은 질문을 도구 없이·도구를 쥐여주고 두 번 물어 그 차이를 실측으로 봅니다. 이어서 최소 호출의 요청·응답 JSON 원문에서 출발해 역할·토큰·무상태·비용을 잡고, 통합 레이어 위에서 교체·폴백·캐싱·라우팅을 쓰고, 도구 호출 왕복을 JSON 원문으로 해부합니다. 예제 31종을 전부 실행하며, 세션 끝에서 여행 플래너 에이전트가 처음으로 도구를 듭니다."
date: 2026-08-21
category: foundation
slides: ["session-05"]
...
교재는 `examples/03_tools/` 10종이고, 이 장 끝에서 **에이전트가 도구를 듭니다**. **10종을 전부 실행합니다.** 아래에는 10종 전부의 실제 실행 결과가
실려 있습니다.
...
### 6-12. 전체 그림: 요청에서 답까지, 도구 호출의 한 바퀴
...
### 6-13. 코드 포인트: 예산 계산 도구가 스키마로 실리는 지점
