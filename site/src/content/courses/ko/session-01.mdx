...
> A반(교육 선행형) 1주차 라이브 세션 교안입니다.  
> 세션은 멘토 시연 중심으로 진행하며, 수강생이 세션 중 직접 손을 움직이는
> 구간은 개인 repo 개설 실습뿐입니다.  
...
- "이 시대에 [[agent|에이전트]]를 만드는 방식": AI 코딩 도구에 요구사항을 지시하고, 생성물을 검증·수정하는 사이클을 직접 목격
- 5개 프로젝트 트랙이 "같은 구조에서 무엇을 갈아끼우는가"의 차이라는 감각
- 개인 GitHub repo (학습 목표 작성 완료)

**1주차 시연의 성격.** 1주차 시연의 목적은 코드 이해가 아니라, "14주 뒤 내가
만들 것의 축소판"과 "그것을 만드는 방식"을 눈으로 확인하는 것입니다. 코드 한
줄 한 줄이 아니라 구조와 사이클이 남으면 성공입니다.
...
flowchart TD
    subgraph common["공통 역량 구간 (1~9주)"]
        direction LR
        W1["1주<br/>전주기 이해"] --> W2["2주<br/>개발환경"] --> W3["3주<br/>LLM·프롬프트"] --> W4["4주<br/>데이터·RAG"] --> W5["5주<br/>API·도구"] --> W6["6주<br/>루프·하네스"] --> W7["7주<br/>트랙 패턴"] --> W8["8주<br/>FastAPI"] --> W9["9주<br/>자유 UI"]
    end
...
**매주 템플릿 형식의 실습이 제공됩니다.** 3주차 [[llm|LLM]] 호출, 4주차
[[rag|RAG]] 검색, 6주차 에이전트, 8주차 API 서버 등 주차별 템플릿은 그대로 개인 프로젝트의 부품으로
...
flowchart TB
    subgraph template["전면 — 템플릿 실습 (매주 제공)"]
        direction LR
        T1["1주<br/>주문 에이전트"] --> T3["3주<br/>LLM 호출"] --> T4["4주<br/>RAG 검색"] --> T6["6주<br/>에이전트"] --> T8["8주<br/>API 서버"] --> T9["9주~<br/>멘티 코드 리뷰<br/>또는 추가 템플릿"]
    end
...
- **[[litellm|LiteLLM]]이 Platform 역할**: Vertex 같은 매니지드 플랫폼이 해주는 "여러 모델을 한 인터페이스로"를 오픈소스 라이브러리 하나로 얻습니다.
...
| 4주 | Data (수집·정제 → 임베딩 → RAG) |
| 5\~7주 | Agents (도구 → 루프·하네스 → 트랙별 패턴) |
...
다룹니다. 별도 시연은 없고, 사다리의 실물은 바로 다음 라이브 빌드에서
확인합니다.
...
- 1단만 있으면: 말은 잘 듣지만 아는 게 없는 모델입니다. 지어내기 시작합니다.
...
이 흐름을 코드와 대조하며 읽어 보세요. 사다리 3단, 하네스의 씨앗입니다.
...
- 도구를 부를지, 몇 번 부를지, 언제 멈출지를 **코드가 아니라 모델이 결정합니다.** 이것이 에이전트와 일반 API 호출의 차이입니다.
- 이 while 루프가 하네스의 씨앗입니다. 지금은 루프뿐이지만, 여기에 가드레일·검증·메모리가 붙으면 하네스 엔지니어링이 됩니다.

### 5-5. 저장소 설계: v0.1에서 v1.0으로

**저장소**: [week01-order-agent](https://github.com/2026-ai-service-engineering-a/week01-order-agent)는
조직 소유이고, template repository로 지정되어 있습니다. 시연 시작점은
...
- 모델 문자열은 환경변수 `MODEL`로 분리. 이 문자열만 바꾸면 Claude도 로컬 모델도 됩니다. 왜 중요한지는 3주차에서 다룹니다.
...
시연에서는 이 프롬프트를 PROMPT.md에 한 줄씩 타이핑하며 의도를 설명합니다.
프롬프트 자체가 교재입니다. **요구사항을 정의하는 능력이 곧 설계 능력**이기
때문입니다.
...
- 이 API는 지금 거짓말을 합니다. 세션에서는 이 자리에서 진짜로 만듭니다.
...
- 만들어지는 파일을 따라가며 앞의 두 지도와 연결합니다.
  - 지금 생긴 이 while 루프가 하네스 층입니다 (사다리 연결)
  - 이 `litellm.completion` 한 줄이 앞 그림의 Platform 레이어입니다 (전주기 연결)
...
- 남는 것은 코드가 아니라 사이클입니다. 지시하고, 검증하고, 고칩니다. 14주 동안 배우는 것은 이 사이클을 출하 가능한 품질로 돌리는 법입니다.

### 5-9. 재현할 때 유의할 점

LLM 코딩은 비결정적입니다. 같은 프롬프트를 같은 모델에 줘도 생성 결과가 매번
조금씩 다릅니다.

- **영상과 다른 결과가 나오는 것이 정상입니다.** 파일 이름이나 함수 분해가 달라도 요구사항을 만족하면 성공입니다. `git diff v0.1 v1.0`은 정답이 아니라 참고 구현입니다.
- **에러나 어긋난 구현도 과정의 일부입니다.** 무엇이 어긋났는지 짚어 수정을 지시하는 것이 이 시연의 본체이므로, 한 번에 안 나온다고 잘못된 것이 아닙니다.
- **Gemini 무료 티어에는 rate limit이 있습니다.** 연달아 재시도하다 막히면 잠시 기다렸다 다시 부르거나, 보조 키를 준비합니다.
...
v0.2의 한계가 하나씩 기능이 됩니다. 사다리로 말하면: 세션 메모리는 2단
[[context-engineering|컨텍스트 엔지니어링]]의 첫걸음, [[guardrail|가드레일]]은
3단 [[harness-engineering|하네스 엔지니어링]]의 첫걸음입니다.
...
**보안 관점: [[prompt-injection|인젝션]]은 어디까지 막히나.** "다른 질문을
하면 점원이 거절한다"가 지금 왜 되는지, 그리고 어디까지 믿을 수 있는지 구분해
둡니다.
...
- 요약: **말은 뚫릴 수 있어도, 피해가 나는 경로는 전부 결정적 코드 뒤에
  있습니다.** 출력 검증·본격 가드레일은 하네스 엔지니어링의 주제입니다.
  4주차에 RAG가 붙으면 "문서에 심어 둔 지시"라는 새 인젝션 표면이 생기고,
  6주차 하네스에서 그 간접 인젝션을 재현해 막습니다.
...
- 1주차부터 깔리는 프로젝트 계획 백그라운드 과제와 이어집니다. 매주 시연을 보며 "내 프로젝트라면 무엇을 갈아끼울까"를 생각해 보세요.
