...
- **UI와 API는 별개 컨테이너이고, 대화는 HTTP뿐입니다.** 이것이 헤드리스입니다. UI를 Streamlit에서 Next.js로 갈아끼워도 API는 한 줄도 안 바뀝니다. [[docker-compose|compose]] 네트워크에서 서비스 이름(`api`)이 곧 호스트명이 됩니다 (2주차 app↔db와 같은 원리)
