...
import AgentMap from '../../../../components/AgentMap.astro';
import WeightSplit from '../../../../components/WeightSplit.astro';
import HandoffReplay from '../../../../components/HandoffReplay.astro';
import ParallelTimeline from '../../../../components/ParallelTimeline.astro';
...
```mermaid
flowchart LR
    A["설계 원칙<br/>언제 나누는가"] --> B["supervisor<br/>중앙이 조율"] --> C["handoff<br/>제어권 이양"] --> D["병렬 실행<br/>계획이 역할을 만든다"]
```
...
오늘 배우는 것이 바로 그 신호와 대가, 그리고 나누는 법입니다
...
```mermaid
flowchart LR
    A["신호 셋"] --> B["대가 셋"] --> C["정할 것 넷"] --> D["두 패턴"] --> E["LangGraph<br/>최소 문법"]
```
...
```mermaid
flowchart LR
    subgraph ONE["도구 10개를 한 손에"]
        A["에이전트"] --> t1["조회"] & t2["계산"] & t3["차트"] & t4["검색"] & t5["…6개 더"]
    end
    subgraph TWO["역할마다 필요한 만큼만"]
        B["analyst"] --> u1["조회 4종"]
        C["visualizer"] --> u2["make_chart"]
    end
    ONE -->|"나누면"| TWO
    style A stroke:#d0453f
    style B stroke:#0ca30c
    style C stroke:#0ca30c
```
...
```mermaid
flowchart LR
    subgraph P["시스템 프롬프트 한 문서"]
        a["분석 지침"] <-->|"다툰다"| b["상담 말투"]
        b <-->|"다툰다"| c["차트 규칙"]
    end
    P -->|"한 줄짜리 질문에도 통째로"| Q["매 호출"]
```
...
```mermaid
flowchart LR
    s1["스텝 1<br/>810tk"] --> s2["스텝 2<br/>1,116tk"] --> s3["…"] --> s6["스텝 6<br/>1,552tk"]
    raw["분석 원자료"] -.->|"상담 단계까지 따라온다 (오염)"| s6
    style raw stroke:#d0453f
```
...
```mermaid
flowchart TB
    S["나눈다"] --> a["왕복 증가<br/>호출 3회 → 5회"]
    S --> b["상태 전달 설계<br/>전부? 요약만?"]
    S --> c["라우팅 실수<br/>새 실패 모드"]
```
...
#### 선언한 그래프는 스스로 그린다
::sub[`print(app.get_graph().draw_mermaid())` 한 줄이면 그래프가 자기 그림을 내놓습니다]

```mermaid
flowchart TD
    S(["__start__"]) --> sup["supervisor"]
    analyst["analyst"] -->|"add_edge"| sup
    sup -.->|"route"| analyst
    sup -.->|"route"| counselor["counselor"]
    sup -.->|"route"| E(["__end__"])
```

- 위 선언과 1:1입니다. **실선은 고정 엣지, 점선은 조건부 엣지.**
- 문서용 그림을 따로 그려서 코드와 어긋나는 일이 없습니다.
  그래프가 곧 그림이고, 그림이 곧 코드입니다.
...
```mermaid
flowchart LR
    A["신호 셋"] -->|"보이면, 대가를 알고"| B["나눈다"]
    B --> C["모아야 하면<br/>supervisor"]
    B --> D["담당이 바뀌면<br/>handoff"]
```
...
```mermaid
flowchart TB
    S["supervisor"] --> A["analyst"] & V["visualizer"] & C["counselor"]
```
...
```mermaid
flowchart LR
    Q["같은 질문"] --> one["단일 v0.1"] --> M["실측 비교"]
    Q --> sup["supervisor v0.2"] --> M
```
...
```mermaid
flowchart LR
    A["구조·데이터"] --> B["v0.1<br/>기준점"] --> C["v0.2<br/>실측"] --> D["사고 재현 ①"] --> E["v1.0<br/>하이브리드"]
```
...
```mermaid
flowchart LR
    ONE["한 덩어리<br/>지침 + 도구 + 문맥"] --> g1["지침<br/>자기 문장만"]
    ONE --> g2["도구<br/>필요한 것만"]
    ONE --> g3["문맥<br/>요약만 위로"]
```
...
```mermaid
flowchart LR
    S["supervisor"] -.->|"건너뜀"| A["analyst<br/>조회 도구 4종"]
    S ==>|"직행"| C["counselor<br/>도구 없음"]
    C --> X["조회 없는 상담문<br/>출력만 보면 그럴듯"]
    style X stroke:#d0453f
    style C stroke:#d0453f
```
...
```mermaid
flowchart LR
    M["make_chart(months, title)"] --> DB["db 직접 조회"] --> OK["막대 = 원본"]
    L["LLM이 옮겨 적은 수치"] -.-> NG["어긋나도 모른다"]
    style OK stroke:#0ca30c
    style NG stroke:#d0453f
```
...
```mermaid
flowchart LR
    A["v0.1 단일<br/>지침 1,105자"] -->|"나누면"| B["역할별 199자<br/>호출 3 → 5회"]
    B --> C["요약만 위로"] --> D["흔들리는 자리는<br/>규칙이 잠근다"]
```
...
```mermaid
flowchart LR
    T["접수"] -->|"Command"| B["요금 담당"] -->|"Command"| R["해지방어 담당"] --> U["고객에게 직접 답한다"]
```
...
```mermaid
flowchart LR
    A["두 패턴<br/>비교"] --> B["v0.1<br/>분류만"] --> C["v0.2<br/>Command"] --> D["사고 재현 ②"] --> E["v0.3<br/>기준 셋"] --> F["v1.0<br/>RAG 부품"]
```
...
#### 직접 눌러 보기: 담당 넷과 도구 배분

<AgentMap variant="handoff" />
...
```mermaid
flowchart LR
    A["transfer_to_*"] --> B["Command(goto)"] --> C["담당이 대화를 쥔다"] --> D["체크포인터에 남는다"]
    D -.->|"매 턴 덮어쓰면"| X["매 턴 접수부터"]
    style X stroke:#d0453f
```
...
```mermaid
flowchart LR
    P["planner"] --> W1["worker"] & W2["worker"] & W3["worker"] --> I["integrator"]
```
...
```mermaid
flowchart LR
    S["SPEC 한 장"] --> P["태스크 3분해"] --> W["병렬 코딩"] --> M["머지 · 테스트"] --> D["docker로 뜬다"]
```
...
```mermaid
flowchart LR
    A["경계 긋기"] --> B["v0.1<br/>순차"] --> C["v0.2<br/>Send·worktree"] --> D["v0.3<br/>비용 게이트"] --> E["사고 재현 ③"] --> F["v1.0<br/>전체 실행"]
```
...
```mermaid
flowchart TB
    K["CONTRACT · 공유 규약"] --> b["backend → app.py"]
    K --> f["frontend → index.html"]
    K --> t["tests → test_api.py"]
    H["골격·하네스<br/>Dockerfile · git · 빌드"] -.-|"모델이 만지지 않는다"| K
```
...
```mermaid
flowchart LR
    subgraph FIX["미리 그린 엣지"]
        s1["상태"] --> w1["worker A"]
        s1 --> w2["worker B"]
    end
    subgraph RUN["Send"]
        p["계획: tasks N건"] -->|"N개만큼 띄운다"| wn["worker × N<br/>각자 페이로드"]
    end
    FIX --> RUN
```
...
#### 직접 눌러 보기: 어디가 LLM이고 어디가 코드인가

<AgentMap variant="parallel" />
...
```mermaid
flowchart LR
    I["실패 로그"] --> FX["fixer<br/>한 파일씩 ≤2"] --> J["다시 판정"]
    J -.->|"그래도 실패"| RP["재계획 ≤1"] -.-> STOP["포기"]
    style STOP stroke:#d0453f
```
...
```mermaid
flowchart LR
    L["6주차의 루프"] --> S["supervisor"] & H["handoff"] & P["병렬"]
    S & H & P --> N["다음: 서버 안으로<br/>FastAPI"]
```
