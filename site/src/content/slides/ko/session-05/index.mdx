...
description: "5주차 라이브 세션 슬라이드: AI 모델과 AI 서비스의 구분과 도구의 정의, examples/01_api 11종으로 LLM API의 바닥, examples/02_litellm 10종으로 통합 레이어, examples/03_tools 11종으로 tool calling 심화, 그리고 에이전트가 처음으로 도구를 드는 지점까지"
...
```mermaid
flowchart LR
    Z["3장<br/>모델과 서비스<br/>도구란 무엇인가"] --> A["4장<br/>LLM API의 본질<br/>01_api 11종"]
    A --> B["5장<br/>LiteLLM 통합 레이어<br/>02_litellm 10종"]
    B --> C["6장<br/>Tool calling 심화<br/>03_tools 10종"]
    C --> D["에이전트가<br/>도구를 든다"]
```
...
*모델은 뇌다. 뇌만으로는 오늘 날씨를 모른다*

</Slide>
<Slide>

### 3장이 하는 일
::sub[예제를 열기 전에 좌표부터 잡습니다]

- 우리가 "AI"라고 뭉뚱그려 부르는 것은 **두 층**입니다
- 같은 질문 다섯 개를 **도구 없이 한 번, 도구를 쥐여주고 한 번** 물어 봅니다
- 그 차이를 메우는 것이 **도구**입니다. 정의를 여기서 박아 둡니다
...
- **AI 모델**: Claude Opus·Fable, GPT, Gemini.
  **텍스트를 넣으면 텍스트가 나오는 함수 하나**
- **AI 서비스**: ChatGPT, Claude Code, Codex, Cursor.
  모델 하나를 가운데 두고 **대화 화면·도구·루프·방어선**을 둘러 붙인 제품
...
**능력의 경계를 긋는 것은 모델의 성능이 아니라 붙어 있는 도구 목록입니다**.
...
학습이 끝난 뒤의 일, 내 컴퓨터의 파일, 지금 이 순간의 시각은 저 상자 안에 없습니다.

</Slide>
<Slide>

#### ② 도구가 있으면
::sub[같은 뇌인데 고리가 바깥까지 뻗습니다]
...
바뀐 것은 모델이 아니라 **모델이 부를 수 있는 것이 생겼다**는 점뿐입니다.
...
- **모델은 대체로 모르는 것을 압니다**: 날씨·시각·파일 목록에는
  "접근할 수 없다"고 물러섭니다. 안전한 실패입니다
- **위험한 쪽은 거절하지 않는 질문입니다**: `3847 × 29183`에
  **112,267,021**을 망설임 없이 내놨습니다. 정답은 112,267,001입니다
- **도구를 붙였다고 매번 부르지는 않습니다**: 여행 추천에서는
  `tool_calls`가 비어 있었습니다
...
모델은 **자기 안에 없는 것을 못 하고**,

**못 한다는 것을 늘 알려주지는 않으며**,
...
### 도구란 무엇인가
::sub[모델이 이름으로 부를 수 있게 등록해 둔 우리 코드의 함수]
...
모델에게 보이는 것은 이 셋이 전부입니다. **함수 본문은 보이지 않습니다**.
...
- ChatGPT가 "검색 중…"을 띄우는 순간: **웹 검색 도구**가 불린 것입니다
- Claude가 첨부 문서를 읽고 표를 고치는 순간: **파일 읽기·쓰기 도구**입니다
- Claude Code가 코드를 고치고 테스트를 돌리는 순간: Read·Edit·Bash이고,
  그것을 반복하는 루프가 6주차의 **ReAct**입니다

전부 같은 구조입니다. 모델이 요청하고, 제품의 코드가 실행하고, 되먹입니다.
...
- **프롬프트에 적어 둔 지시**: "계산은 정확히 해줘"는 **부탁**이고,
  계산기 도구는 **실행**입니다. 부탁은 확률로, 실행은 코드로 지켜집니다
- **모델 안의 지식**: 오사카 명소를 아는 것은 학습의 결과지 조회가 아닙니다
- **RAG로 미리 넣어 준 문서**: 경계에 있습니다. 4주차에서 검색이 추려 넣어 준
  것은 **우리가** 넣어 준 것이지 모델이 요청한 것이 아닙니다
...
- 도구는 **이름·설명·입력 스키마**로 등록한 우리 코드의 함수다
- 모델은 실행하지 못한다. **요청할 뿐이고 실행은 우리 몫**이다
...
**6장에서 저 자리가 채워지는 순간 에이전트가 시작됩니다**.
...
- 타입까지 묶는 방법은 **6장의 구조화 출력**에서 잇습니다.
...
### 5장이 하는 일
::sub[4장이 이미 써 온 completion()의 정체를 밝힙니다]
...
`examples/03_tools/` 11종

*4장에서 비어 있던 tool_calls 자리가 채워집니다*
...
### 6장이 하는 일
::sub[4장에서 비어 있던 tool_calls 자리가 채워집니다]

- 도구 호출 왕복을 **JSON 원문으로 해부**합니다.
- `examples/03_tools/` **11종을 전부 실행**합니다.
- Pydantic 모델 하나가 스키마와 검증을 겸하는 구조를 익힙니다.
- 세션 끝에서 에이전트가 **처음으로 도구를 듭니다**.
...
4. 에러와 환각 (되먹임, 검증)
5. 구조화 출력과 도구 선택
6. 도구가 없다면? (같은 계산을 두 방식으로)
7. 전체 그림
8. 코드 포인트: 에이전트에 도구가 실린다
...
3장의 그림을 이 장의 문법으로 다시 봅니다

</Slide>
<Slide>

#### 같은 계산을 두 방식으로
::sub[examples/03_tools/11_calc_with_without.py]

```text
=== 질문: 3+4+(5 곱하기 2) 를 계산해줘 ===   (정답 17)

① 도구 없이   → "…괄호 안을 먼저: 5 × 2 = 10 … 계산 결과는 17입니다"   ○ 맞음

② 계산기 있음 → 모델의 요청  calculator({"expression": "3+4+(5*2)"})
                우리의 실행  {"result": 17}
                최종 답      "계산 결과는 17입니다"                    ○ 맞음
```

**두 답이 같습니다.** 도구 없는 쪽은 풀이까지 단정하게 적었습니다.

</Slide>
<Slide>

#### 자릿수를 키우면
::sub[같은 코드, 같은 도구, 숫자만 크게]

```text
=== 질문: 48627 곱하기 9314에 1250을 더하면? ===   (정답 452,913,128)

① 도구 없이   → "452,905,678"                                        ✗ 틀림
② 계산기 있음 → calculator({"expression": "48627 * 9314 + 1250"})
                → 452913128                                          ○ 맞음
```

- 형식은 여전히 단정하고 **"확실하지 않다"는 신호가 없습니다**
- 다시 돌리면 **틀린 값 자체가 달라집니다** (준비 중 452,902,628도 나왔습니다)
...
쉬운 질문에서 도구가 할 일이 없어 보였다면,
아직 틀릴 만큼 어렵지 않았던 것입니다
...
이 랩의 에이전트는 도구를 **딱 1왕복만** 씁니다.  
여러 도구를 오가며 반복하는 루프는 6주차 랩에서 완성됩니다.
