...
description: "6주차 라이브 세션 슬라이드: ReAct 루프와 트레이스, 스텝 한도와 컨텍스트 팽창, Plan-Execute·재계획·Reflexion·ReWOO, 인젝션 방어와 데이터 경계, 권한·감금·필터, budget guard, MCP 시연 4종, v0.3에서 v1.2까지"
...
- **루프를 단다**: reasoning → action → observation (v0.3)
- **루프를 견준다**: Plan-Execute · 재계획 · Reflexion · ReWOO (v0.4)
- **루프를 가둔다**: 인젝션 방어 · 권한 · 감금 · budget guard (v1.0)
- **도구를 표준으로**: MCP 시연 4종 (v1.1\~v1.2)
...
### 랩 이어받기
::sub[5주차 랩의 연장입니다. 첫 태그가 v0.2인 이유]
...
`agent/`와 `tests/`는 5주차의 v0.2와 같고,
`examples/`에 이번 주 몫 24종이 더 들어 있습니다.
...
| v0.3 | ReAct 루프 완성: 여행 플래너가 처음 동작 | 17개 | 3장 |
| v0.4 | 고급 루프: Reflexion 재시도 장착 | 21개 | 4장 |
| v1.0 | 하네스 완성: 인젝션 방어 + budget guard | 33개 | 5장 |
| v1.1\~v1.2 | MCP 시연 4종 (stdio·HTTP 전송) | 33개 | 6장 |
...
- 장 끝에서 에이전트가 **v0.3**, 처음으로 제대로 동작합니다.
...
- 장 끝에서 에이전트에 Reflexion 재시도가 실려 **v0.4**가 됩니다.
...
**v0.4가 정확히 그 구조입니다.** 생성자 = 3장의 ReAct 루프
...
- 5주차 4장의 수용 테스트에서 이 가드로 실측한 에이전트 풀런 비용이
  $0.0026\~$0.0104였습니다.
