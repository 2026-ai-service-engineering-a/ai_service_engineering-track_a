...
| 주차 | 주제 | 세션 실습 포커스 | 산출물 | VOD 사전 시청 |
| --- | --- | --- | --- | --- |
| 1 | AI 서비스 전주기 이해 | 과정 조망, 엔지니어링 사다리, 프로젝트 트랙 소개, 개인 repo 개설 | 학습 목표 + 개인 repo | 캡스톤 데모 (전체 그림) |
| 2 | Python AI 개발환경 | devcontainer 구동, uv/pip, GitHub 워크플로, Codex/Claude 활용 세팅 | 동작하는 개발 환경 | S1 환경 셋업 |
| 3 | LLM API와 프롬프트 | LiteLLM 멀티 프로바이더 호출, 프롬프트 패턴, structured output(instructor) | LLM 호출 실습 코드 | S1 전체 |
| 4 | 데이터 수집·정제 | CSV/PDF/웹/API 수집, pandas 정제, 한국어 PDF 함정 대응 | 정제 데이터셋 | S2 데이터 수집·문서 로딩 |
| 5 | 임베딩·벡터DB | sentence-transformers(`bge-m3`), Chroma add/query, 유사도 검색 | 검색 실습 코드 | S2 청킹·임베딩·Chroma |
| 6 | RAG 기본 구현 | chunking → retrieval → generation → 출처 표시, 간단 검색 평가 | 미니 RAG | S2 mini RAG·검색 평가 |
| 7 | 자동화·분석·추천 패턴 | 트랙별 미니 실습 (에이전트/분석/추천·분류), 트랙 가결정 | 트랙별 실습 + 트랙 가결정 | S3 에이전트 빌드 |
| 8 | FastAPI 서비스화 | AI 기능 API화, 입력 검증·에러 처리·스트리밍 | AI API 서버 | S5 FastAPI 서빙 |
| 9 | 자유 UI 구현 | Streamlit / Gradio / Next.js / Swagger UI 선택 (라이브 브리핑 제공) | 웹 데모 초안 | — (VOD 범위 밖) |

### 10\~14주 — 개인 프로젝트 구간

| 주차 | 주제 | 세션 포커스 | 산출물 | VOD 연계 |
| --- | --- | --- | --- | --- |
| 10 | 개인 프로젝트 기획 | 문제 정의, MVP 범위, 데이터·스택 선정, **기획 리뷰 통과** | 프로젝트 기획서 | — |
| 11 | 프로젝트 MVP 개발 | 핵심 AI 기능 + API/UI 연결, **배포 파이프라인 사전 개통** | 동작 MVP + 1차 배포 | S5 서빙 Docker |
| 12 | 배포·평가·개선 | 무료 클라우드 배포 안정화, 테스트셋·품질 개선 로그 | 데모 URL + 평가 로그 | S4 평가·검증·관찰 |
